select distinct concat(pn.given_name," ", ifnull(pn.family_name,'')) as name,
 ...
 from
     visit v join person_name pn on v.patient_id = pn.person_id and pn.voided = 0 and v.voided=0
             ...
             inner join queue_v7 q on pi.identifier = q.identifier and q.status=1
             left outer join visit_attribute va on va.visit_id = v.visit_id and va.attribute_type_id = (
         select visit_attribute_type_id from visit_attribute_type where name="Admission Status"
     )
 where
     v.date_stopped is null
   and obs.concept_id = (select concept_id from concept_name where name="Opd Consultation Room" and concept_name_type="FULLY_SPECIFIED" limit 1)
   and obs.voided = 0
